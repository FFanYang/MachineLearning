...
names = ['id', 'diagnosis', 'radius_mean', 'texture_mean', 'perimeter_mean', 'area_mean','smoothness_mean', 'compactness_mean', 'concavity_mean',]
         ...
         # 'campactness_worst','concavity_worst', 'concave points_worst', 'symmetry_worst', 'fractal dimension_worst']
...
def make_plots_all(mlp, target_train, target_test, predictors_test,predictors_train):
    ...
    for i in range(max_iter):
        mlp.fit(predictors_train, target_train) #ce shi
    ...
    plt.title("Accuracy and Loss over Interations", fontsize=16)
...
predictors_test,predictors_train):
    ...
    # a single hidden layer with k  = 25 neurons  - NN
    clf = MLPClassifier(random_state=1, hidden_layer_sizes=(25),
 max_iter=100,)
